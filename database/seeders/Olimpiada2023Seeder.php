    ...
    public function run(): void
    {
        DB::transaction(function () {
            ...
            $responsableUser = Usuario::create([
                'nombre' => 'Carlos', 'apellido' => 'Perez', 'ci' => '9988776', 'email' => 'carlos.perez@test.com', 'password' => 'password123'
            ]);
            $responsableUser->asignarRol('Responsable Area', $olimpiada->id_olimpiada);

            $evaluadorUser = Usuario::create([
                'nombre' => 'Lucia', 'apellido' => 'Mendez', 'ci' => '6655443', 'email' => 'lucia.mendez@test.com', 'password' => 'password123'
            ]);
            ...
            // 7. Crear Area-Nivel, Fases y Parámetros para Matemáticas
            $areaNivelMatematicas = AreaNivel::create([
                'id_area' => $areas->firstWhere('nombre', 'Matemáticas')->id_area,
                'id_nivel' => $niveles->first()->id_nivel,
                'id_olimpiada' => $olimpiada->id_olimpiada,
                'activo' => false, // La competencia ya pasó
            ]);

            $faseClasificatoria = Fase::create(['nombre' => 'Clasificatoria', 'orden' => 1, 'id_area_nivel' => $areaNivelMatematicas->id_area_nivel]);
            $faseFinal = Fase::create(['nombre' => 'Final', 'orden' => 2, 'id_area_nivel' => $areaNivelMatematicas->id_area_nivel]);

            $parametro = Parametro::create([
                'nota_max_clasif' => 100, 'nota_min_clasif' => 51, 'cantidad_max_apro' => 10, 'id_area_nivel' => $areaNivelMatematicas->id_area_nivel
            ]);
            $this->command->info('Configuración de Fases y Parámetros creada.');

            // 8. Crear Competidores
            $competidoresData = [
                ['datos' => json_encode(['nombre' => 'Ana', 'apellido' => 'Gomez']), 'id_institucion' => $institucion1->id_institucion],
                ['datos' => json_encode(['nombre' => 'Juan', 'apellido' => 'Roca']), 'id_institucion' => $institucion1->id_institucion],
                ['datos' => json_encode(['nombre' => 'Sofia', 'apellido' => 'Luna']), 'id_institucion' => $institucion2->id_institucion],
                ['datos' => json_encode(['nombre' => 'Mateo', 'apellido' => 'Soliz']), 'id_institucion' => $institucion2->id_institucion],
            ];

            $competidores = [];
            foreach ($competidoresData as $data) {
                $competidores[] = Competidor::create(array_merge($data, [
                    'id_area_nivel' => $areaNivelMatematicas->id_area_nivel
                ]));
            }
            $this->command->info('Competidores creados.');
            ...
            $competidorDescalificado = Competidor::create([
                'datos' => json_encode(['nombre' => 'Pedro', 'apellido' => 'Infante']),
